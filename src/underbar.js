(function() {
  ...
  _.uniq = function(array) {
    ...
    _.each(newArray, function(element, outerIndex){
      var a = outerIndex;
      _.each(newArray, function(element, innerIndex){
        var b = innerIndex;
        if (a !== b){
  ...
  _.map = function(collection, iterator) {
    ...
    if(collection.constructor === Array){
      ...
      for(var i = 0; i < collection.length; i++){
        var index = i;
        mapped.push(iterator(collection[index], index, collection));
      }
    }if(collection.constructor === Object){
      for(var key in collection){
  ...
  _.reduce = function(collection, iterator, accumulator) {
    var initalizing = accumulator === undefined;
    _.each(collection, function(item){
      if(initalizing){
        accumulator = item;
        initalizing = false;
